...
export async function createProSubscription(admin) {
  ...
  return { confirmationUrl: billingResult.confirmationUrl, error: null };
}
...
export async function hasActiveProPlan(admin, billing) {
  const { activeSubscription } = await getProSubscription(billing, admin);
...
export async function cancelProSubscription(billing, admin) {
  ...
  if (!activeSubscription?.id) return { cancelled: false };
